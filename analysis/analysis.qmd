...
table1["Variable"] = table1["Variable"].replace(
    {
        "mobile_traffic_per_sub": "Mobile traffic per subscription",
        "5g_penetration_rate": "Estimated 5G penetration rate",
        "mobile_broadband_penetration": "Mobile broadband penetration",
...
for region in REGION_ORDER:
    sub = latest[latest["region"].eq(region)]
    if sub.empty:
        continue
    ax.scatter(
...
display(Image(filename=str(REPORT_FIGURES / "eda_5g_vs_traffic.png")))
plt.close(fig)
```
...
display(Image(filename=str(REPORT_FIGURES / "eda_regional_trends.png")))
plt.close(fig)
```
...
```{python}
#| label: fig-static-3
#| fig-cap: "Year-over-year traffic growth rate versus estimated 5G penetration across country-years. The dashed line marks the 20% penetration threshold. High-growth outliers are concentrated below 20% penetration, reflecting catch-up dynamics in emerging markets rather than 5G-driven acceleration. At higher penetration levels, growth rates stabilize but traffic levels are substantially higher in absolute terms (see Figure 1). The growth-rate correlation is weak (r = -0.05), while the level correlation between 5G penetration and traffic per subscription is 0.48.† Based on synthetic 5G subscription estimate; see Methods for derivation."
#| fig-width: 6.5
#| fig-height: 4
fig, ax = plt.subplots(figsize=(6.5, 4))
...
display(Image(filename=str(REPORT_FIGURES / "eda_correlation.png")))
plt.close(fig)
```
...
```{python}
#| label: fig-static-4
#| fig-cap: "Random Forest model results. Left: variable importance scores show that estimated 5G penetration rate is the single strongest predictor of mobile traffic per subscription, followed by mobile broadband penetration and log GDP per capita. Right: predicted versus actual values (R² = 0.48, RMSE = 4.48 GB/sub/month). The model underestimates the highest-traffic outliers, likely reflecting idiosyncratic market conditions not captured by country-level features.† Based on synthetic 5G subscription estimate; see Methods for derivation."
#| fig-width: 10
#| fig-height: 4
...
display(Image(filename=str(REPORT_FIGURES / "rf_results.png")))
plt.close(fig)
```
